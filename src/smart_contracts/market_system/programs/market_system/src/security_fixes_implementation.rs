// Implementation of security fixes for smart contracts
use anchor_lang::prelude::*;
use anchor_spl::token::{self, Token};

// Apply high severity fixes first
// H-01: Multi-signature requirement for market resolution
// H-02: Proper error handling for token transfers

// Update market resolution to require multiple signatures
pub fn resolve_market(ctx: Context<ResolveMarket>, args: ResolveMarketArgs) -> Result<()> {
    let market = &mut ctx.accounts.market;
    let resolution_proposal = &mut ctx.accounts.resolution_proposal;
    ...
    require!(
        ctx.accounts.platform_state.is_resolution_committee_member(&ctx.accounts.resolver.key()),
        PredictionMarketError::UnauthorizedResolver
    );
    
    // Add approval if not already approved
    if !resolution_proposal.approvals.contains(&ctx.accounts.resolver.key()) {
        resolution_proposal.approvals.push(ctx.accounts.resolver.key());
    }
    
    // Check if proposal has enough approvals
    if resolution_proposal.is_approved() {
        // Resolve market
        market.status = MarketStatus::Resolved { 
            winning_option: args.winning_option_index 
        };
        
        // Record resolution time
        market.resolution_time = Clock::get()?.unix_timestamp;
...
// Update reward claiming with proper error handling
...
// M-01: Reentrancy guard
// M-02: Time buffer for market expiration
// M-03: Slippage protection for token swaps

// Add reentrancy guard to prediction placement
...
// Apply low severity fixes
// L-01: Add event emissions
// L-02: Remove hardcoded gas values
// L-03: Add comprehensive input validation
// L-04: Standardize error handling
// L-05: Add emergency pause for individual markets
...
// Add emergency pause for individual markets
